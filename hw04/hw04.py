...
def balanced(m):
    ...
    leftmobile=left(m)
    rightmoblie=right(m)
    if is_mobile(end(leftmobile)):
        if balanced(end(leftmobile))==False:
            return False
    if is_mobile(end(rightmoblie)):
        if balanced(end(rightmoblie))==False:
            return False
    if total_mass(end(leftmobile))*length(leftmobile)!=total_mass(end(rightmoblie))*length(rightmoblie):
        return False
    return True
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        return max(max_path_sum(branches(t)[0]),max_path_sum(branches(t)[1]))+label(t)
