...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    s=[paragraphs[i] for i in range(0,len(paragraphs)) if select(paragraphs[i])]
    if len(s)>k:
        return s[k]
    else:
        return ''
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def func(st):
        st=split(lower(remove_punctuation(st)))
        for i in subject:
            for j in st:
                if i==j:
                    return True
...
def accuracy(typed, source):
    ...
    if typed_words==[] and source_words==[]:
        ...
        s=[typed_words[i] for i in range(0,len(typed_words)) if typed_words[i]==source_words[i]]
        return len(s)/len(typed_words)*100
    else:
        s=[typed_words[i] for i in range(0,len(source_words)) if typed_words[i]==source_words[i]]
        return len(s)/len(typed_words)*100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for s in word_list:
        if typed_word==s:
            return typed_word
    num=[diff_function(typed_word,s,limit) for s in word_list]
    a=min(num)
    if a<=limit:
...
def furry_fixes(typed, source, limit):
    ...
    def helper(i,change):
        if change>limit:
            return change
        if len(typed)<=i:
            return change-len(typed)+len(source)
        if len(source)<=i:
            return change+len(typed)-len(source)
        if typed[i]==source[i]:
            return helper(i+1,change)
        else:
            return helper(i+1,change+1)
    return helper(0,0)
    # END PROBLEM 6
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times=[[timestamps_per_player[i][j+1]-timestamps_per_player[i][j] for j in range(0,len(timestamps_per_player[i])-1)] for i in range(0,len(timestamps_per_player))]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    def helper(player,word):
        summary=[times[i][word] for i in player_indices]
        if times[player][word]==min(summary) and all(times[player][word]<times[i][word] for i in range(0,player)):
            return True
        else:
            return False
    return [[words[j] for j in word_indices if helper(i,j)] for i in player_indices]
    # END PROBLEM 10
